...
echo -e "\n \033[0;39m Register: git log mine \033[0m \n"
git config --global alias.lgm "log --color --graph --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr) %C(bold blue)<%an>%Creset%C(yellow)[%ce]%Creset' --abbrev-commit --author="xingming.dai""

echo -e "\n \033[0;39m Register: git log record \033[0m \n"
git config --global alias.lgr "log --color --graph --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr) %C(bold blue)<%an>%Creset%C(yellow)[%ce]%Creset' --abbrev-commit --author="xingming.dai" --since=1.day.ago"
